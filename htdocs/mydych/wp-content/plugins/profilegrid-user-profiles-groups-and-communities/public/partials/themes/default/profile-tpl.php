      ...
      <div class="pm-profile-title-header pm-dbfl">
        ...
        <div class="pm-profile-title pm-difl pm-pad10">
          ...
          <div class="pm-user-name pm-dbfl pm-clip">
              ...
              echo wp_kses_post( apply_filters( 'profilegrid_field_privacy_username', $display_name, $uid ) );
              ?>
          </div>
          <?php endif;?> 
          ...
          <div class="pm-user-group-name pm-dbfl pm-clip">
              <a href='<?php echo esc_url( $group_page_link ); ?>'>
                  ...
                  <?php echo esc_html( $groupinfo->group_name );?>
              </a>
               <?php 
               $total_assign_group = count( array_unique( $gid ) );
               if ( ! empty( $gid ) && is_array( $gid ) && $total_assign_group > 1 ) :
                   if ( $total_assign_group > 2 ) { 
                       $group_count_String = esc_html__( 'more groups', 'profilegrid-user-profiles-groups-and-communities' );
                   } else {
                       $group_count_String = esc_html__( 'more group', 'profilegrid-user-profiles-groups-and-communities' );
                   }
               ?>
              <div class="pg-more-groups">
                  <a onclick="pg_open_group_tab()">+<?php echo esc_html( count( array_unique( $gid ) ) - 1 . ' ' . $group_count_String ); ?> </a>
              </div>
               <?php endif;?>
               <a></a> 
    ...
    <div class="pm-popup-container pm-update-image-container pm-radius5">
	  ...
	  <div id="changePic" class="" style="display:none">
            <form id="cropimage" method="post" enctype="multipart/form-data" action="<?php echo esc_url( admin_url( 'admin-ajax.php' ) );?>">
                <?php
                // Use class object for nonce with fallback
                if ( isset( $basicfunctions ) && method_exists( $basicfunctions, 'pm_render_nonce_field' ) ) {
                    $basicfunctions->pm_render_nonce_field( 'pm_profile_update_action', 'pm_profile_nonce' );
